...
public class Executor {
    ...
    public int executeUpdate(String statement, Object param, String methodType) throws Exception {
        /**
         * 数据库连接信息硬编码
         */
        String driver = configuration.getDriver();
        ...
        Connection conn = DriverManager.getConnection(url, username, password);
        //3.创建SQL语句对象Statement，填写SQL语句
        Map<String,Object> sqlHandler = sqlHandler(sqlStr);
        System.out.println(sqlHandler.get("sql").toString());
        PreparedStatement ps = conn.prepareStatement((String) sqlHandler.get("sql"));
        if ("delete".equals(methodType)){
            Integer paramDelete = (Integer) param;
            ps.setInt(1, paramDelete);
        }else {
            Class<?> clazz = Class.forName(parameterType);
            //反射获取当前类的所有方法
            Method[] methods = clazz.getMethods();
            List<String> parames = (List<String>) sqlHandler.get("fields");
            for (int i = 0; i < parames.size(); i++) {
                for (Method method : methods) {
                    String methodName = method.getName();
                    //判断方法的名称，与set+列名相等，那么就把列名对应的值设置到当前对象的set方法中
                    if (("get"+parames.get(i)).equalsIgnoreCase(methodName)){
                        //把列名column对应的值,设置到对象的set方法中,给属性赋值
                        Object field = method.invoke(param);
                        System.out.println(i + 1 + " = " + field + ";");
                        if (field instanceof Integer){
                            ps.setInt(i + 1, (int)field);
                        }else if (field instanceof String){
                            ps.setString(i + 1, (String) field);
                        }
                        break;
                    }
                }
    ...
    public Map<String,Object> sqlHandler(String sql){
        ...
        while (i < sql.length()){
            if ('#' == sql.charAt(i)){
                ...
                fields.add(sql.substring(start, i));
        ...
        res.put("sql",stringBuilder.toString());
